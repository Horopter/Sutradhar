...
# Launch Masterbolt - Complete Startup Script
# Starts all required services: Sutradhar, Optimus, Convex, and Masterbolt Frontend
...
echo -e "${BLUE}🚀 Launching Masterbolt - Teacher in Your Pocket${NC}"
echo "========================================================"
echo ""
...
# Start Masterbolt Frontend
echo ""
echo -e "${BLUE}4️⃣  Starting Masterbolt Frontend (Port 3777)...${NC}"
cd "$PROJECT_ROOT/apps/masterbolt"
if [ ! -d "node_modules" ]; then
    echo "   Installing dependencies..."
...
pnpm dev > /tmp/masterbolt.log 2>&1 || npm run dev > /tmp/masterbolt.log 2>&1 &
MASTERBOLT_PID=$!
echo "   PID: $MASTERBOLT_PID"
echo "   Logs: /tmp/masterbolt.log"
echo "   Note: First build may take 60-90 seconds..."

# Wait for Masterbolt
echo "   Waiting for frontend compilation..."
for i in {1..120}; do
    if curl -s http://localhost:3777 2>&1 | grep -q "Masterbolt\|Continue as Guest" 2>/dev/null || ! curl -s http://localhost:3777 2>&1 | grep -q "Nuxt dev server is starting" 2>/dev/null; then
...
echo "   Masterbolt: $MASTERBOLT_PID"
echo ""
echo -e "${BLUE}📝 Log Files:${NC}"
echo "   Sutradhar: /tmp/sutradhar.log"
echo "   Optimus:   /tmp/optimus.log"
echo "   Convex:    /tmp/convex.log"
echo "   Masterbolt: /tmp/masterbolt.log"
echo ""
echo -e "${YELLOW}💡 To stop all services:${NC}"
echo "   kill $SUTRADHAR_PID $OPTIMUS_PID $CONVEX_PID $MASTERBOLT_PID"
echo ""
echo -e "${YELLOW}💡 Or use:${NC}"
echo "   pkill -f 'sutradhar|optimus|convex|masterbolt'"
echo ""

# Save PIDs
echo "$SUTRADHAR_PID $OPTIMUS_PID $CONVEX_PID $MASTERBOLT_PID" > /tmp/masterbolt-pids.txt
...
echo -e "${GREEN}🎉 Masterbolt is launching!${NC}"
