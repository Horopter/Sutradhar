 ...
 * For distributed systems, use rate-limiter-redis.ts instead
 */
class RateLimiter {
...
// Import distributed rate limiter if available
import { distributedRateLimit } from './rate-limiter-redis';
import { cache } from '../cache';

// Use distributed rate limiting if Redis is available, otherwise use in-memory
const useDistributed = (cache as any).isRedis === true;

const createRateLimiter = (config: { windowMs: number; maxRequests: number; keyGenerator?: (req: Request) => string }) => {
  if (useDistributed) {
 ...
 * Automatically uses Redis-based rate limiting if available
 */
