...
export async function getGithubRepo(owner: string, repo: string): Promise<GitHubRepository> {
  ...
  return ((result.result as unknown as GitHubRepository) || ((result as any).data as unknown as GitHubRepository) || (result as unknown as GitHubRepository)) as GitHubRepository;
}
...
export async function updateGithubIssue(
  ...
  return ((result.result as unknown as GitHubIssue) || ((result as any).data as unknown as GitHubIssue) || (result as unknown as GitHubIssue)) as GitHubIssue;
}
...
export async function addGithubIssueComment(
  ...
  const result = await safeAction("github-add-comment", { owner, repo, issueNumber, body }, async () => {
    const composio = createComposioClient();
    const connectedAccountId = getGitHubConnectedAccountId();
    const entityId = getComposioUserId();
