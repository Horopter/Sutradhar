...
describe('Answer Service', () => {
  ...
  describe('answerQuestion', () => {
    it('should generate answer with retrieval context', async () => {
      ...
      (llmService.chat as jest.Mock).mockResolvedValue(mockLLMResponse);

      const result = await answerService.answerQuestion('session123', 'What is X? How can I use Slack to send messages?', 'Greeter');
    ...
    it('should handle retrieval failures gracefully', async () => {
      ...
      const result = await answerService.answerQuestion('session123', 'How can I create a calendar event?');
    ...
    it('should check guardrails before responding', async () => {
      ...
      (retrievalService.search as jest.Mock).mockResolvedValue({ snippets: [], mocked: false });
      (llmService.chat as jest.Mock).mockResolvedValue(mockLLMResponse);

      const result = await answerService.answerQuestion('session123', 'What is X?');
